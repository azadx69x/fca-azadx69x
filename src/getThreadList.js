  ...
  return {
    ...
    userInfo: edges.map((d) => {
      ...
      return {
        ...
        vanity: p.username,
        url: p.url,
        thumbSrc: p.big_image_src?.uri || p.big_image_src?.url
          || p.profile_picture?.uri || p.profile_picture?.url
...
module.exports = function (defaultFuncs, api, ctx) {
  ctx._groupThreadIDs = ctx._groupThreadIDs || new Set();
  ...
  return async function getThreadList(limit, timestamp = null, tags = ["INBOX"]) {
    ...
    try {
      ...
      const threads = nodes.map(formatThreadGraphQLResponse);
      ...
      for (const thread of threads) {
        if (thread?.threadID != null) {
          ...
          ctx._threadMetadata.set(threadID, {
            threadID,
            isGroup,
          ...
          if (isGroup) {
            ctx._groupThreadIDs.add(threadID);
          } else {
            ctx._groupThreadIDs.delete(threadID);
          }
        }
      }
      return threads;
