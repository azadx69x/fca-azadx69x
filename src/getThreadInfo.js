...
function formatThreadGraphQLResponse(data) {
  ...
  return {
    ...
    userInfo: (messageThread.all_participants?.edges || [])
      .filter((d) => d?.node?.messaging_actor?.id)
      .map((d) => {
        const actor = d.node.messaging_actor;
        const image = actor.big_image_src || actor.profile_picture || actor.profilePic;
        return {
          id: actor.id,
          name: actor.name,
          ...
          url: actor.url,
          thumbSrc: image?.uri || image?.url || actor.profile_picture_url || null,
